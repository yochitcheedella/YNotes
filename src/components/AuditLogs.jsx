      ...
      <div className="glass-card rounded-2xl p-6 shadow-xl space-y-6">
        <div className="flex justify-between items-center">
          ...
          <h2 className="text-lg font-semibold text-white flex items-center gap-2">
            <span className="material-symbols-outlined text-cyberBlue-400">security</span>
            Security Audit Logs
          </h2>
          <div className="w-16" />
        </div>

        <div className="bg-[#050505] rounded-xl border border-blue-900/30 overflow-hidden">
          {isLoading ? (
            <div className="p-8 text-center text-blue-400/50 font-mono text-sm animate-pulse">Scanning security logs...</div>
          ) : logs.length === 0 ? (
              ...
              {logs.map((log) => (
                <li key={log.id} className="p-4 hover:bg-white/5 transition-colors flex items-start justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-blue-300/60 font-mono">{log.device_info}</p>

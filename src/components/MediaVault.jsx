    ...
    <main className="relative z-10 pt-20 px-4 max-w-4xl mx-auto pb-24 space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-xs text-blue-300/40 mt-1">Symmetrically encrypted attachment binaries stored securely on cloud nodes.</p>
        </div>
        <span className="px-3 py-1 bg-cyberBlue-950/50 border border-cyberBlue-500/20 text-cyberBlue-400 font-mono text-[10px] uppercase tracking-widest rounded-lg flex items-center gap-1.5 shadow-[0_0_15px_rgba(59,130,246,0.1)]">
        ...
        {['images', 'voice', 'documents'].map(tab => (
          <button
            ...
            className={`px-6 py-3.5 text-xs font-mono uppercase tracking-wider border-b-2 transition-all capitalize
              ${activeTab === tab 
                ? 'border-cyberBlue-500 text-cyberBlue-400' 
                : 'border-transparent text-blue-300/40 hover:text-white'
              }
            `}
          >
            {tab}
      ...
      <div 
        ...
        className="border-2 border-dashed border-blue-500/20 bg-slate-950/20 rounded-2xl p-8 flex flex-col items-center justify-center text-center group hover:border-cyberBlue-500/40 transition-colors"
      >
        <span className="material-symbols-outlined text-[42px] text-blue-400/30 group-hover:text-cyberBlue-400 transition-colors mb-3">cloud_upload</span>
        <p className="text-xs text-white font-semibold">Drag and drop files to encrypt</p>
        <p className="text-[10px] text-blue-300/30 font-mono mt-1">Supports PNG, JPG, MP3, PDF (Max 15MB)</p>
        <label className="mt-4 px-4 py-2 bg-cyberBlue-600 hover:bg-cyberBlue-500 text-white text-[10px] font-mono uppercase tracking-wider rounded-lg cursor-pointer transition-colors">
          Browse Files
      ...
      <section className="mt-6">
        {activeTab === 'images' && (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {IMAGES.map(img => (
              <div key={img.id} className="glass-card rounded-xl overflow-hidden group border border-blue-500/10 relative">
                {/* Thumb */}
                ...
                <div className="p-3 bg-slate-950/40">
                  ...
                  <p className="text-[10px] text-blue-300/40 font-mono mt-0.5">{img.size} • {img.date}</p>
                </div>
              </div>
            ))}
          </div>
              ...
              <div key={vn.id} className="glass-card rounded-xl p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-blue-300/40 font-mono">{vn.duration} • {vn.size} • {vn.date}</p>
                  </div>
                </div>
                <button 
                  onClick={() => alert('Decrypting and streaming voice recording securely...')}
                  className="p-2 rounded-lg bg-cyberBlue-950/50 hover:bg-cyberBlue-900/50 border border-cyberBlue-500/25 text-cyberBlue-400 hover:text-white transition-colors"
                >
              ...
              <div key={doc.id} className="glass-card rounded-xl p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-blue-300/40 font-mono">{doc.size} • {doc.date}</p>
                  </div>
                </div>
                <button 

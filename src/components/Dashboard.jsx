    ...
    <main className="relative z-10 pt-20 px-4 max-w-4xl mx-auto pb-24">
      ...
      <section className="mb-8">
        <div className="glass-card rounded-xl p-6 shadow-lg">
          ...
          <div className="grid grid-cols-7 text-center mb-2 text-xs font-mono text-diaroAccent-300/40 uppercase tracking-widest">
              ...
              return (
                <div 
                  ...
                  className={`p-2 flex flex-col items-center justify-center rounded-lg cursor-pointer transition-all duration-150
                    ${!item.currentMonth ? 'text-diaroAccent-100/10 cursor-default' : 'text-diaroAccent-100 hover:bg-white/10'}
                    ${isSelected ? 'bg-diaroAccent-600/30 text-diaroAccent-400 border border-diaroAccent-500/40' : ''}
                  `}
                >
      ...
      <section className="space-y-6 relative pl-8 border-l border-slate-800/60">
        <div className="flex items-center justify-between pl-4">
          ...
          <span className="text-xs font-mono text-diaroAccent-300/60">
        ...
        {filteredNotes.length > 0 ? (
          filteredNotes.map((note) => (
            <div key={note.id} className="relative flex gap-4 animate-fade-in">
              <div className="absolute -left-12 w-8 h-8 rounded-full bg-slate-900 border-2 border-diaroAccent-500 flex items-center justify-center shrink-0 mt-1 z-10 text-sm">
                {note.mood === 'happy' ? '😊' : note.mood === 'sad' ? '😔' : '🌿'}
              </div>
              <div 
                className="glass-card flex-1 rounded-xl p-5 hover:border-diaroAccent-500/30 transition-all cursor-pointer group"
              ...
              >
                ...
                <p className="text-sm text-diaroAccent-100/60 line-clamp-2 mb-4">
                  {note.content}
                </p>
                <div className="flex gap-2">
          ...
          <div className="text-center py-12 text-diaroAccent-300/30 font-mono text-sm">
      ...
      <button 
        onClick={onNewNote}
        className="fixed bottom-24 right-6 w-16 h-16 bg-diaroAccent-600 text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-50 group hover:shadow-[0_0_20px_rgba(184, 115, 51,0.5)]"
      >
